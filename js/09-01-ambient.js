...
async function main() {
  ...
  const vertSrc = await fetch("glsl/09-01.vert").then((r) => r.text());
  const fragSrc = await fetch("glsl/09-01.frag").then((r) => r.text());
  ...
  const fragUniforms = {
    u_lightColor: new Float64Array([1.0, 1.0, 1.0]),
  };
